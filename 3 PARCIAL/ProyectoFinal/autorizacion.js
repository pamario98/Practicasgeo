auth.onAuthStateChanged( user =>{
    ...
    if(user){
        ...
        db.collection('ProductoApple').onSnapshot(snapshot =>{
            obtieneProductoApples(snapshot.docs);
            configuraMenu(user);
        }, err => {
...
formaregistrate.addEventListener('submit',(e)=>{
    ...
    auth.createUserWithEmailAndPassword(correo,contrasena).then( cred =>{
        ...
        return db.collection('usuariosapple').doc(cred.user.uid).set({
...
function mensajeError(codigo) {
    ...
    switch(codigo) {
        case 'auth/wrong-password':
          mensaje = 'Su contraseÃ±a no es correcta';
          break;
        case 'auth/user-not-found':
            mensaje = 'El usuario no existe o el correo no esta registrado';
            break;
        case 'auth/weak-password':
            mensaje = 'ContraseÃ±a dÃ©bil debe tener al menos 6 caracteres';
            break;
        default:
            mensaje = 'OcurriÃ³ un error al ingresar con este usuario';
      }
    return mensaje;
  }
